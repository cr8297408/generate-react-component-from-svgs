...
const processSVGs = async () => {
  ...
  for (const file of files) {
    if (path.extname(file) === '.svg') {
      // Leer el contenido del SVG
      let svgCode = await fs.readFile(path.join(inputFolder, file), 'utf-8');

      // Reemplazar o agregar el atributo viewBox
      const viewBoxMatch = svgCode.match(/viewBox="[^"]*"/);
      if (viewBoxMatch) {
        svgCode = svgCode.replace(
          viewBoxMatch[0],
          'viewBox="0 0 41.114 41.114"'
        );
      } else {
        svgCode = svgCode.replace('<svg', '<svg viewBox="0 0 41.114 41.114"');
      }
